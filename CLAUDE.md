...
- **発表用スライド**（`AI詰将棋トレーナー_発表用.pptx`、2026-07追加・現状は下記の通り）: 表紙＋テーマ／背景／
  目的／開発内容／使用技術／開発環境／画面紹介（7画面：ホーム・問題集・プレイ画面・盤面の見方・ヒント機能・
  デイリーチャレンジ・結果とランキング）／工夫した点／苦労した点／アピールポイント／実演／課題／今後の展望／
  まとめ、の全21枚構成（表紙を除き本文スライドは1〜20の連番、ページ番号丸バッジもタイトルの連番と一致済み）。
  全21枚に発表メモ（ノートペイン）を日本語の話し言葉スクリプトとして記入済み。画面紹介の文言・実演スライドの
  構成が変わった場合は、対応するスライドのノートも合わせて更新すること。
  - **フォント／文字サイズ**: 全テキスト「BIZ UDGothic」（UDフォント、Morisawa作・OFLライセンスでGoogle
    Fonts経由でも配布、可読性重視で日本語プレゼン資料の定番）で統一。文字サイズも役割ごとに統一済み
    （タイトル見出し40pt、本文の箇条書き32pt、装飾ボックスの主要ラベル30pt・補足ラベル24pt、表紙タイトル60pt
    など）。**注意**: 似た名前の「BIZ UDPゴシック」（プロポーショナル版、Microsoft 365/Windows同梱）とは
    別ファミリーで、フォント名が一致しないと明朝体などへフォールバックする。発表機材にBIZ UDGothicが
    入っていない場合は事前に https://fonts.google.com/specimen/BIZ+UDGothic から入れておくか、本番PCで
    開いて崩れがないか確認すること。
  - **画面紹介スライドのスクリーンショット**: 最新UI（実際に生成されたタイトル・修正済みの段/筋表記・現行の
    ナビ構成）を`npm run dev`起動→Playwrightで撮影したものに差し替え済み。結果・ランキングのスライドのみ、
    Supabase未接続だと`/ranking`が空表示になるため、代わりに1手詰を解いた際のAI解説つき正解モーダルを採用。
    画像の表示枠は元より横幅を約13%拡大し（画像とキャプションの余白を削ってバランスを調整）、丸角の半径も
    デフォルト（短辺の約16.7%）だと拡大後の画像の角がはみ出すため6%に縮小済み。
  - **レイアウトの制約**: このpptxの本文テキストボックスは`autofit=spAutoFit`かつ縦位置が全て固定絶対値
    （各シェイプが独立していて後続シェイプの位置を押し出す仕組みではない）ため、1行に収まらず折り返すと
    直下の要素やスライド下端と重なる。本文を編集する際は必ず1行に収まる長さに収めること（Pillowで
    `BIZ UDGothic`の実測フォント幅を計算し、テキストボックス幅に収まるか確認する運用）。また、タイトル
    文字の直下にある装飾下線（Rectangle 3）は固定位置のため、タイトルのフォントサイズを変える場合は
    下線側の位置も合わせて調整すること。
  - **検証方法**: このMacにはPowerPointはあるがAppleScript/GUI自動化での画像書き出しが機能しないため、
    `soffice --headless --convert-to pdf`（Homebrewでインストール済みのLibreOffice）でPDF化し、PyMuPDF
    (`fitz`)でPNGにレンダリングして目視確認する運用としている。スライド内容やレイアウトを変更した際は
    必ずこの方法で全ページを再確認すること。
  - **既知の注意点（インシデント）**: 2026-07のセッション中、スクリーンショット撮影のためAppleScript経由で
    PowerPointアプリからこのファイルを開いたところ、直後にスライドが1枚（「実演」）消失する事象が発生した
    （原因未特定）。直前のgitコミットから該当スライドのXMLを復元して復旧済みだが、**このファイルをPowerPoint
    などのGUIアプリで開いた状態のまま、同時にスクリプト（python-pptx等）で編集しないこと**。GUIで開く必要が
    ある場合は他の編集をすべて終えてから単独で開き、都度ファイルの枚数・内容をpython-pptxで確認する。
  - このpptxを生成した元スクリプト（python-pptx製、アイコン画像はnpm経由のsimple-icons/devicon＋Homebrewの
    librsvgで生成）、撮影に使ったPlaywrightスクリプト、フォント幅計測用の`measure.py`はいずれもセッション用
    スクラッチパッド内にのみ存在しリポジトリには含まれていない。今後レイアウトやスクリーンショットを
    再調整する場合は、都度同様のスクリプトを組み直すか、pptx内部のXMLを直接編集する必要がある。
