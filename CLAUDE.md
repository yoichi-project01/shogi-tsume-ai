...
- **発表用スライド**（`AI詰将棋トレーナー_発表用.pptx`、2026-07追加）: テーマ／背景／目的／開発内容／使用技術／
  開発環境／システム構成／画面紹介（7画面）／工夫した点／苦労した点／アピールポイント／実演／課題／
  今後の展望／まとめ、の全22枚構成。使用技術スライドはNext.js/TypeScript/Tailwind CSS/Supabase/Netlifyの
  各公式ロゴ（simple-icons、CC0ライセンス）を実際に画像として埋め込み済み。このpptxはpython-pptxで組んだ
  生成スクリプトで作成しているが、そのスクリプト自体はセッション用スクラッチパッド内にのみ存在しリポジトリ
  には含まれていないため、今後レイアウトを再調整する場合は都度python-pptxで組み直すか、pptx内部のXMLを
  直接編集する必要がある。画面紹介スライドの8枚のスクリーンショットは実装より少し古いUI（旧タイトル表記
  「N手詰（自動生成）」・旧ランキング3タブ・旧盤面座標表記・旧ナビ構成など）のまま流用しているため、
  最新UIで撮り直すとより正確になる。
